...
function createRemoveTodoListButton(index) {
  ...
  todoListRemoveButton.addEventListener("click", (e) => {
    ...
    const todoContainer = document.querySelector(
      `div[data-display="${todoListIndex}"]`
    );
    ...
    while (
      document.querySelector(
        `.todo-container[data-display="${parseInt(todoListIndex) + 1}"]`
      )
    ) {
...
function createAddTodoItemButton() {
  ...
  addButton.addEventListener("click", (e) => {
    ...
    const todoItemElement = displayTodoItem(newTodoItem, todoList.length - 1);
...
function createTitleEditButton() {
  ...
  editButton.addEventListener("click", (e) => {
    ...
    document.querySelector("#todo-list-title").replaceWith(titleInputText);

    document
      .querySelector("#edit-title-button")
...
function createEditCancelButton() {
  ...
  cancelButton.addEventListener("click", (e) => {
    ...
    document.querySelector("#title-input").replaceWith(todoListTitleText);

    /*replacing the confirm button with the edit button, and removing the cancel button*/
    document
      .querySelector("#confirm-edit-button")
...
function displayTodoItem(todoItem, index) {
  ...
  let labelTextArray = ["", "", "Priority: ", "Due Date: ", "Done"];
...
function createTodoItemRemoveButton(index) {
  ...
  removeButton.addEventListener("click", (e) => {
    ...
    document
      .querySelector(`div[data-todo-item="${e.target.dataset.removeItem}"]`)
      .remove();
  });
